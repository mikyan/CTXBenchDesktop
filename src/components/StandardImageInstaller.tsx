...
import { availableImageTasks, imageStatus, selectedProjectImages, standardImageError, type StandardImagePlan } from "../lib/standard-images";
...
export function StandardImageInstaller({ dataset, name, onClose, onExperiment }: { dataset: string; name: string; onClose: () => void; onExperiment?: (selection: { taskIds: string[]; profile?: CompanyProfileRecord }) => void }) {
  ...
  const install = () => {
    setConfirm(false);
    void run(async () => setJob(await workerRequest<OperatorJob>("/intranet/operations/standard-images", "POST", { dataset, taskIds: selected, confirmed: true, ...(profileId ? { profileId } : {}) })));
  };
  return <Modal title={`${t("Install project images")} · ${name}`} onClose={onClose} busy={busy}>
    ...
    <fieldset disabled={busy || Boolean(active)}><CompanyProfilePicker value={profileId} preparationOnly onChange={(next) => { setProfile(next); setPlan(undefined); setJob(undefined); setSelected([]); }} /></fieldset>
    <p>{t("For a company registry, first save image mappings in Settings → Company profiles, then select that same profile here and in your experiment. Checks do not download layers or call a model. Registry availability can change; only installed images are ready for the next preparation step.")}</p>
    ...
    {plan && <>
      <fieldset disabled={busy || Boolean(active)} className="standard-image-selection">
        ...
        <div className="toolbar"><button type="button" className="button secondary" disabled={!visible.length} onClick={() => void run(async () => setJob(await workerRequest<OperatorJob>("/intranet/operations/image-check", "POST", { dataset, taskIds: visible.map((row) => row.id), confirmed: true, ...(profileId ? { profileId } : {}) })))}>{t("Check matching tasks in registry")}</button><button type="button" className="button secondary" onClick={() => { const available = new Set(availableImageTasks(plan)); setSelected(visible.filter((row) => available.has(row.id)).map((row) => row.id)); }}>{t("Select only tasks with available images")}</button></div>
      ...
      <section><h3>{t("2. Review required images")}</h3>
        ...
        <div className="form-actions"><button type="button" className="button primary" disabled={busy || Boolean(active) || !selected.length || incompatible || blocked || !plan.storage.ready} onClick={() => setConfirm(true)}>{t("Install selected project images")}</button>
          {onExperiment && <button type="button" className="button secondary" disabled={busy || Boolean(active) || !selected.length || missing.length > 0 || incompatible} onClick={() => onExperiment({ taskIds: selected, profile })}>{t("Create experiment with selected tasks")}</button>}
