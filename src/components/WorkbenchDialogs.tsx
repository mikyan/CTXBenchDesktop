...
export function PreparationDialog({ kind, onClose, onComplete }: { kind: "context" | "constraints" | "manual"; onClose: () => void; onComplete: () => void }) {
  ...
  return <Modal title={t(kind === "manual" ? "Import package" : kind === "context" ? "Generate context" : "Mine constraints")} busy={busy} warnOnClose onClose={onClose}>
    ...
    {error && <FormError>{error}</FormError>}<button className="button primary" disabled={busy || !taskId} onClick={() => void submit()}>{busy ? t("Working…") : t("Submit")}</button>
  </Modal>;
}
export function RunDialog({ run, onClose }: { run: BenchmarkRun; onClose: () => void }) {
  ...
  return <Modal title={`${run.taskId} · ${run.arm}`} onClose={onClose}>
    ...
    {error && <FormError>{error}</FormError>}<pre className="log-view">{content}</pre>{more && <button className="button secondary" onClick={() => void read(next)}>{t("Load more")}</button>}
...
export function PackageDialog({ id, onClose }: { id: string; onClose: () => void }) {
  ...
  return <Modal title={t("View package")} onClose={onClose}>
    <code>{id}</code>{error && <FormError>{error}</FormError>}
