...
export function Modal({ title, children, onClose }: { title: string; children: ReactNode; onClose: () => void }) {
  ...
  return <dialog ref={ref} aria-labelledby={titleId} className="workbench-dialog" onCancel={(event) => { event.preventDefault(); onClose(); }}><header><h2 id={titleId}>{title}</h2><button className="icon-button" aria-label={t("Close")} onClick={onClose}><X size={18} /></button></header><div className="workbench-form">{children}</div></dialog>;
}
