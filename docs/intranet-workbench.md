...
入口：**基础设施 → 公司内网适配工作台**。需要同时更新桌面前端和 Worker 镜像；旧 Worker 不支持 `/v1/intranet`。升级前让已有实验结束或暂停，保留原数据卷，不要清空实验数据。
...
向导最后一步和基础设施工作台都可真实自检。每次最多 20 个任务，必须提供 evaluator-only `goldPatch`，不调用 Agent。每阶段离线容器运行，2 CPU／4 GiB／10 分钟。
