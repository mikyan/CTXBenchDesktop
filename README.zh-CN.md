...
v0.1.8 修复官方 CTXBench 大用例导致启动即提示“单个用例定义不能超过 10MB”、无法创建用例的问题。列表异常不再锁住独立创建入口；缺失或损坏的旧数据单独提示。需同时更新桌面端与应用镜像，保留原数据目录，不用删除或重新下载评测集。详见[版本说明](docs/releases/v0.1.8.md)。
