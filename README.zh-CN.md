...
v0.1.10 增加带影响范围确认的用例、评测集和结果删除，优化镜像与实验向导，修复非 root 镜像权限及已完成运行的日志查看。真实 OpenCode + MiMo 自创用例配对已通过独立评分。需同时更新桌面端与应用镜像，保留原数据目录。详见[版本说明](docs/releases/v0.1.10.md)。

v0.1.7 新增可编辑的用例库与评测集组合、不可变运行快照、GitHub CI 判分、自定义非 Pi 编码命令、远端镜像下载和四步镜像制作向导。自定义 Agent 的 Token 用量未知不影响功能测试。验收边界详见[v0.1.7 说明](docs/releases/v0.1.7.md)及[工作台使用指南](docs/workspace-navigation.md)。
...
当前源码还支持删除用例、评测集、已结束的实验及单轮完整对照组，删除前会弹窗说明影响范围并再次确认。保留历史快照及共享文件，不自动清理磁盘。[查看删除说明](docs/data-deletion.md)。此功能需配套的桌面端与评测服务镜像，尚未随现有 Release 发布。
...
- **错误诊断、完整日志与证据**：实验、知识库准备队列和运行详情提供[执行诊断与完整运行日志](docs/live-container-logs.md)。Agent 启动前的配置、Git、镜像、目录权限和构建错误，也会显示失败步骤、原始异常和排查建议。支持每秒实时刷新、从头分页查看全部已保存日志、切换阶段、暂停刷新和导出当前窗口；重试保留之前的错误记录。原有 `workflow.json`、`setup.log` 等证据仍可手动查看。需要配套更新镜像；旧版本缺失的日志无法补回，部分官方单测详情仍只写入评分证据。
