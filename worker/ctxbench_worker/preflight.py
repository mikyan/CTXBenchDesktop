...
def estimate(spec: ExperimentSpec, tasks: list) -> dict:
    ...
    custom_tasks = sum(bool(getattr(task, 'agent', None)) for task in tasks)
    unmetered_solves = custom_tasks * len(spec.arms) * spec.repeats
    ...
    allowance = {
        ...
        'solver': (solves - unmetered_solves) * spec.model.max_tokens,
    ...
    return {'tasks': count, 'runs': solves, 'contextKeys': keys, 'builderInvocations': builders,
            'customCommandTasks': custom_tasks, 'unmeteredSolverRuns': unmetered_solves,
            'customCommandTokenUsage': 'unknown' if custom_tasks else None,
            'tokenAllowanceExcludesCustomCommands': bool(custom_tasks),
